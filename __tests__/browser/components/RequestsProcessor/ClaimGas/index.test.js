...
describe('<ClaimGas />', () => {
  ...
  const addClaimLoadedListener = (callback) => {
    addLoadedListener(store, callback, `sessions.${sessionId}.${claimKey}-${requestId}`);
  };
  ...
  describe('when claim fails', () => {
    ...
    beforeEach((done) => {
      ...
      console.log(get(store.getState(), `${spunkyKey}.sessions.${sessionId}.${claimKey}-${requestId}`));
    ...
    it.only('rejects', () => {
      // wrapper.update();
      expect(onReject).toHaveBeenCalledWith('Could not claim GAS.');
      expect(onResolve).not.toHaveBeenCalled();
    });
  });
