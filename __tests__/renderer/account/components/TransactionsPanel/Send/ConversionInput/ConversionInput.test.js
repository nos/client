...
describe('<ConversionInput />', () => {
  it('renders the asset input placeholder', () => {
    ...
    expect(wrapper.find('.assetInput input').prop('placeholder')).toEqual(
      'Enter NOS amount'
    );
  });
  ...
  it('renders the currency input placeholder', () => {
    const wrapper = mountContainer();
    expect(wrapper.find('.currencyInput input').prop('placeholder')).toEqual(
      'Enter USD amount'
    );
  });
  ...
  describe('when editing the asset amount', () => {
    it('updates the currency amount', () => {
      const wrapper = mountContainer();
      wrapper
        .find('.assetInput input')
        .simulate('change', { target: { value: '50' } });
      expect(wrapper.find('.currencyInput input').prop('value')).toEqual(
        '2.50'
      );
    });
  ...
  describe('when editing the currency amount', () => {
    ...
    it('rounds to the permitted number of asset decimals', () => {
      ...
      wrapper
        .find('.currencyInput input')
        .simulate('change', { target: { value: '0.48' } });
